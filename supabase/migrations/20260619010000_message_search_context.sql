drop function if exists public.search_messages(text, uuid, integer);
...
create or replace function public.get_conversation_message_context(
...
as $$
  ...
  limited_messages as (
    ...
    where message.conversation_id = target_conversation_id
      and public.can_access_conversation(message.conversation_id, auth.uid())
      and message.created_at <= target.created_at
...
grant execute on function public.get_conversation_message_context(uuid, uuid, integer) to authenticated;
