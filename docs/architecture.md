...
- Finish the v0.7.5 search UX feedback gate: verify search scope labels, result counts, selected-filter summaries, feedback status, and record the result in `docs/v0.7.5-search-ux-trial-feedback.md`.
- Keep GitHub Actions green, confirm Vercel Production app/PWA assets, and clean `codex.v073.*` / `codex.v074.*` / `codex.v075.*` plus matching `codex-v073-*` / `codex-v074-*` / `codex-v075-*` temporary data through `docs/test-data-cleanup-sop.md`.
