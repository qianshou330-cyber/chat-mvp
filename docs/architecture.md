...
- Use `docs/v0.8.1-public-beta-launch-checklist.md` as the current 2026-07-01 public beta launch gate.
- Keep `docs/v0.8.0-production-ops-handoff.md` as the current production-hardening record: backup/restore rehearsal, sanitized app-error checks, Web Push health, A/B/C permission smoke, and temporary cleanup.
- Continue using `docs/test-data-cleanup-sop.md` for `codex.v073.*` through `codex.v081.*` and matching `codex-v073-*` through `codex-v081-*` temporary data.
- Public beta registration is open by product decision; compensate with visible safety copy, daily production patrol, and fast P0/P1 rollback rather than an access-code gate.
